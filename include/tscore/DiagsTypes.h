...
public:
  ...
  {
    return this->on(DiagsTagType_Debug) && ctl.ptr()->on;
  }
  ...
  /// Print the log message only if control is enabled.
  void
  log(DbgCtl const &ctl, DiagsLevel level, const SourceLocation *loc, const char *fmt, ...) const TS_PRINTFLIKE(5, 6)
  {
    if (on(ctl)) {
      va_list ap;
      va_start(ap, fmt);
      print_va(ctl.ptr()->tag, level, loc, fmt, ap);
      va_end(ap);
    }
  }

  void
  log_va(DbgCtl const &ctl, DiagsLevel level, const SourceLocation *loc, const char *fmt, va_list ap)
  {
    if (on(ctl)) {
      print_va(ctl.ptr()->tag, level, loc, fmt, ap);
    }
