...
{
template <>
struct Enabled<T1, T2> : public Yes<T1, T2, compare_function> {};
}
}
...
namespace cmp_op
{
  ...
  template <typename T1, typename T2, int (*Cmp)(const rm_r_t<T1> &, const rm_r_t<T2> &)> struct Yes {
    struct X {
      using XX = bool;
    };
    struct XD {
    ...
    static int
    cmp(const rm_r_t<T1> &op1, const rm_r_t<T2> &op2)
    {
      return Cmp(op1, op2);
    }
  };
  ...
  //
  template <typename T, int (*Cmp)(const rm_r_t<T> &, const rm_r_t<T> &)> struct Yes<T, T, Cmp> {
    struct X {
      using XX = bool;
    };
    static int
    cmp(const rm_r_t<T> &op1, const rm_r_t<T> &op2)
    {
      return Cmp(op1, op2);
    }
  };
...
auto
operator==(const T1 &op1, const T2 &op2) -> typename ts::cmp_op::Enable<T1, T2>::X::XX
{
...
template <typename T1, typename T2>
auto
operator==(const T2 &op2, const T1 &op1) -> typename ts::cmp_op::Enable<T1, T2>::XD::XX
{
  return ts::cmp_op::Enable<T1, T2>::cmp(op1, op2) == 0;
}
template <typename T1, typename T2>
