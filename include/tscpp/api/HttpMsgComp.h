...
{
public:
  DynamicCharArray() : _size(0) {}

  DynamicCharArray(char *arr, std::size_t size) : _arr(arr), _size(size) {}
  ...
  asStringView() const
  {
    return std::string_view(_arr.get(), _size);
  }
  ...
  release()
  {
    auto s = _size;
    _size  = 0;
    return std::string_view(_arr.release(), s);
  }
