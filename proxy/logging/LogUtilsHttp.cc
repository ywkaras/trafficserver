...
namespace
{
// Get a string out of a MIMEField using one of its member funcitions, and put it into a buffer writer, with its marshaled length
// prepended.
//
void
marshalStr(ts::FixedBufferWriter &bw, const MIMEField &mf, const char *(MIMEField::*get_func)(int *length) const)
{
  ...
  // Marshal and insert prepended length of string.  Add one to it, so none of the bytes in the marshaled sequence can be zero.
  //
  LogUtils::marshalInsert(bw, unsigned(length + 1));

  bw << ts::string_view(data, length);
}
