/** @file
  ...
  Catch-based tests for LogUtilsHttp.h.
...
MIMEHdr::MIMEHdr(const MIMEField *first, int count) : _first(first), _count(count), _idx(0)
{
}

const MIMEField *
MIMEHdr::iter_get_first(MIMEFieldIter *)
{
  return _idx < _count ? _first + _idx : nullptr;
}

const MIMEField *
MIMEHdr::iter_get_next(MIMEFieldIter *)
{
  ++_idx;
...
test(const MIMEField *pairs, int numPairs, const char *asciiResult, int extraUnmarshalSpace = 0)
{
  ...
  int binSize = std::strlen(binBuf) + 1;

  REQUIRE(INK_ALIGN_DEFAULT(binSize) == binAlignSize);

  const char *bp = binBuf;
