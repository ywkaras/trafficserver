...
HttpTransactHeaders::normalize_accept_encoding(const OverridableHttpConfigParams *ohcp, HTTPHdr *header)
{
  ...
  if (normalize_ae == -1) { // Not explicitly configured.

    if (ohcp->normalize_ae_gzip == -1) { // Not explicitly configured.

      normalize_ae = 1; // Enable normalization to gzip by default.

    } else {
      static bool logged = false;

      if (!logged) {
        Note("proxy.config.http.normalize_ae_gzip deprecated, use proxy.config.http.normalize_ae");
        logged = true;
      }

      normalize_ae = ohcp->normalize_ae_gzip ? 1 : 0;
    }
  }
