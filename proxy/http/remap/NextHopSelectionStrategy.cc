...
NextHopSelectionStrategy::Init(ts::Yaml::Map &n)
{
          ...
          if (hosts_list.Type() != YAML::NodeType::Sequence) {
            ...
            for (unsigned int hst = 0; hst < hosts_list.size(); ++hst) {
              ...
              if ((self_host == host_rec->hostname) || mach->is_self(host_rec->hostname.c_str())) {
                if (grp != 0) {
                  throw std::invalid_argument("self host (" + self_host + ") can only appear in first host group");
                }
  ...
  if (ring_mode == NH_PEERING_RING) {
    if ((groups != 1) && (groups != 2)) {
      NH_Error("ring mode is '%s', requires one or two host groups (peering group and an optional upstream group).",
               peering_rings.data());
      return false;
    }
