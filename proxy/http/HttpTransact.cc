...
DbgCtl ip_allow_dbg_ctl("ip_allow");
...
HttpTransact::HandleCacheOpenReadHit(State *s)
{
  ...
  if (send_revalidate) {
    ...
    if (server_up) {
      ...
      TxnDbg(http_trans_dbg_ctl, "CacheOpenReadHit - version %d", http_version.m_version);
...
HttpTransact::handle_forward_server_connection_open(State *s)
{
  ...
  if (s->hdr_info.server_response.version_get() == HTTPVersion(0, 9)) {
    TxnDbg(http_trans_dbg_ctl, "[hfsco] server sent 0.9 response, reading...");
...
HttpTransact::get_ka_info_from_config(State *s, ConnectionAttributes *server_info)
{
  ...
  if (server_info->http_version > HTTPVersion(0, 9)) {
    TxnDbg(http_trans_dbg_ctl, "get_ka_info_from_config, version already set server_info->http_version %d",
  ...
  TxnDbg(http_trans_dbg_ctl, "get_ka_info_from_config, server_info->http_version %d, check_hostdb %d",
...
HttpTransact::check_request_validity(State *s, HTTPHdr *incoming_hdr)
{
  ...
  if (method != HTTP_WKSIDX_TRACE) {
    ...
    TxnDbg(http_trans_dbg_ctl, "[init_stat_vars_from_req] set req cont length to %" PRId64, s->hdr_info.request_content_length);
...
HttpTransact::is_request_valid(State *s, HTTPHdr *incoming_request)
{
  ...
  case BAD_HTTP_HEADER_SYNTAX: {
    TxnDbg(http_trans_dbg_ctl, "[is_request_valid] non-existant/bad header");
    SET_VIA_STRING(VIA_DETAIL_TUNNEL, VIA_DETAIL_TUNNEL_NO_FORWARD);
