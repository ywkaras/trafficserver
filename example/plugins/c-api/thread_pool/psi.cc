...
inline ContData *
contDataGet(TSCont cont)
{
  return static_cast<ContData *>(TSContDataGet(cont));
}
...
transform_handler(TSCont contp, TSEvent event, void *edata ATS_UNUSED)
{
  ...
  if (retval) {
    ...
    if (state == STATE_READ_PSI) {
      ...
      TSMutexUnlock(TSContMutexGet(contp));
      cont_data_destroy(contDataGet(contp));
      TSContDestroy(contp);
      return 1;
    }
  } else {
...
{
  auto data = static_cast<TryLockData *>(TSContDataGet(contp));
