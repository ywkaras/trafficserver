...
dumpMimeField(const MimeField &fld)
{
  ...
  TSReleaseAssert(eq_values_list(fld.valuesGet(), all));
...
dumpMsg(MsgBase &msg)
{
  ...
  if (nFlds) {
    ...
    for (int i = 0; i < nFlds; ++i) {
      ...
      TSReleaseAssert(sameMimeField(currFld, iterFld));
      iterFld = iterFld.next();

      MimeField fFld(msg, currFld.nameGet());
      TSReleaseAssert(fFld.valid());

      dumpMimeField(currFld);
    }
    TSReleaseAssert(!iterFld.valid());
  }
}
...
{
  TSReleaseAssert(msg.type() == MsgBase::Type::REQUEST);

  logFile << std::endl << msgName << ':' << std::endl;
  logFile << "method=" << msg.methodGet() << std::endl;

  int urlLength = msg.absoluteUrl(nullptr, 0);
  TSReleaseAssert(urlLength > 0);
  std::unique_ptr<char[]> url(new char[urlLength]);
  TSReleaseAssert(msg.absoluteUrl(url.get(), urlLength) == urlLength);
  std::unique_ptr<char[]> url2(new char[10]);
  TSReleaseAssert(msg.absoluteUrl(url2.get(), 10) == urlLength);
  if (urlLength <= 10) {
    TSReleaseAssert(std::memcmp(url.get(), url2.get(), urlLength) == 0);
  }
  url2.reset(new char[1000]);
  TSReleaseAssert(msg.absoluteUrl(url2.get(), 1000) == urlLength);
  if (urlLength <= 1000) {
    TSReleaseAssert(std::memcmp(url.get(), url2.get(), urlLength) == 0);
  }
...
{
  TSReleaseAssert(msg.type() == MsgBase::Type::RESPONSE);
...
doCrap(MsgBase &msg, bool add)
{
  ...
  TSReleaseAssert(f.valuesCount() == 3);
  TSReleaseAssert(f.valGet(0) == "one");
  TSReleaseAssert(f.valGet(1) == "two");
  TSReleaseAssert(f.valGet(2) == "three");
  TSReleaseAssert(f.valuesGet() == "one, two, three");

  MimeField fd(f.nextDup());

  TSReleaseAssert(fd.valuesCount() == 1);
  TSReleaseAssert(fd.valGet(0) == "four");
  TSReleaseAssert(fd.valuesGet() == "four");

  if (add) {
    fd.valInsert(0, "Three-And-A-Half");

    TSReleaseAssert(fd.valuesCount() == 2);

    fd.valAppend("five");
    fd.valSet(1, "cuatro");

    TSReleaseAssert(fd.valuesCount() == 3);
    TSReleaseAssert(fd.valGet(0) == "Three-And-A-Half");
    TSReleaseAssert(fd.valGet(1) == "cuatro");
    TSReleaseAssert(fd.valGet(2) == "five");
    fd.valuesSet();
    TSReleaseAssert(fd.valuesGet() == "");
    fd.valuesSet("\talpha\t, beta  , gamma");
    TSReleaseAssert(fd.valuesCount() == 3);
    TSReleaseAssert(fd.valuesGet() == "\talpha\t, beta  , gamma");

    MimeField ld = MimeField::lastDup(fd.msg(), "X-Crap");
    TSReleaseAssert(sameMimeField(fd, ld));
  }
}
