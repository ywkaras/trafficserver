...
# File to be loaded into a string and passed to exec().

# Assumes class 'util' is already defined.
...
class pop:
    ...
    @classmethod
    def makeATS(cls, name, origin):
        if (not isinstance(name, str)) and isinstance(name, list):
            return [cls.makeATS(p, origin) for p in name]
        # name -> ts.Name
        # return <- ts

        ts = cls.Test.MakeATSProcess(name)
        ...
        util.debugOut('makeATS', ts.Name, ts.Variables.Port)
        return ts

    @classmethod
    def enableRoutingLog(cls, ts):
        if isinstance(ts, list):
            ret = []
            for p in ts:
                ret += [cls.enableRoutingLog(p)]
            return ret
        # log format:
        log_fmt_str = ' '.join([
        ...
        return util.addLogging(ts, 'routing.log', 'routing', log_fmt_str)

        #test.Disk.File(logfilename, exists=True).Content = cls.Testers.FileContentCallback(g, 'validate_forwards_logged')

    @classmethod
    def addPort(cls, ts, port_name, ipv6=False):
        if isinstance(ts, list):
            for p in ts:
        ...
        cls.debugOut('addPort', [ts.Name, port_name, ts.Variables[port_name]])

    @classmethod
    def cfgDNS(cls, ts, dns):
        if isinstance(ts, list):
            for p in ts:
                cls.cfgDNS(p, dns)
            return

        util.debugOut('cfgDNS', ts.Name, dns.Name)
    ...
    @classmethod
    def cfgParents(cls, ts, parents, dest_domain):
        if isinstance(ts, list):
            for p in ts:
                cls.cfgParents(p, parents, dest_domain)
            return

        util.debugOut('cfgParents', '{} -> {}'.format(ts.Name, [p.Name for p in parents]))
        ...
        parent_fmt_list = ';'.join([cls.localhost + ':{}'.format(p.Variables.Port) for p in parents])
        ...
        # ts.Ready = cls.When.FileContains(ts.Disk.diags_log.AbsPath, 'healthcheck status to 1 at time')#, len(peers)-1)

        ts.StartupTimeout = 15
        ts.TimeOut = 150

        util.debugOut('/cfgParents', [fname, config_text])
    ...
    def cfgHC(cls, ts, up):
        ...
        if isinstance(ts, list):
            for p in ts:
                cfgHC(p, up)
            return
        util.debugOut('cfgHC', [ts, up])
        hc_conf_file = os.path.join(cls.Test.RunDirectory, "hc_{}.config".format(ts.Name))
        status_file = os.path.join(cls.Test.RunDirectory, "status_{}.html".format(ts.Name))
    ...
    @classmethod
    def waitAllStarted(cls, tr, proc_list, when_cond=None):
    ...
    @classmethod
    def checkAllProcessRunning(cls, proc_list):
        tr = cls.Test.AddTestRun("Check all processes still running.")
    ...
    @classmethod
    def cfgResponse(cls, server, dns, host, path, body="", cc="", resp_code=200):
