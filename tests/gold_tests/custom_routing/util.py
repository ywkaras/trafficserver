...
class util:
    ...
    @classmethod
    def debugOut(cls, func, *kargs):
        dt = str(int(1000 * (time.clock() - cls.start_time)))
        print('{:5} {:<15} -'.format(dt, func), *kargs)

    @classmethod
    def extract(cls, content, prefix, suffix):
    ...
    @classmethod
    def addLogging(cls, ts, filename, name, log_fmt_str):
...
'''
        ...
        cls.debugOut('addLogging', filename)

        return os.path.join(ts.Variables.LOGDIR, filename)

    @classmethod
    def saveLogging(cls, ts, filename):
        ...
        for ts in ts_list:
            ...
            cls.debugOut('move file ', [src_f, dst_f])
            tr.Processes.Default.Command += '; mv {src} {dst}'.format(src_f.filename, dst_f.filename)

    @classmethod
    def logFindStrings(cls, content, substr_list):
        cls.debugOut('logFindStrings', substr_list)
    ...
    @classmethod
    def logContains(cls, log, substr_list):
        def f(cls, content): return logFindStrings(content, substr_list)
    ...
    @classmethod
    def objSearch(cls, obj, key, prefix=[], ignore=None):
        if obj is not dict:
            try:
                methods = dir(obj)
                obj = obj.__dict__
