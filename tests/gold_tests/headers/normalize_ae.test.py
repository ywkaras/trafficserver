...
request_header = {"headers": "GET / HTTP/1.1\r\nHost: www.ae-2.com\r\n\r\n", "timestamp": "1469733493.993", "body": ""}
server.addResponse("sessionlog.json", request_header, response_header)
request_header = {"headers": "GET / HTTP/1.1\r\nHost: www.ae-gzip-0.com\r\n\r\n", "timestamp": "1469733493.993", "body": ""}
server.addResponse("sessionlog.json", request_header, response_header)
request_header = {"headers": "GET / HTTP/1.1\r\nHost: www.ae-gzip-1.com\r\n\r\n", "timestamp": "1469733493.993", "body": ""}
server.addResponse("sessionlog.json", request_header, response_header)
...
def baselineTsSetup(ts):
    ...
    ts.Disk.remap_config.AddLine(
        'map http://www.ae-gzip-0.com http://127.0.0.1:{0}'.format(server.Variables.Port) +
        ' @plugin=conf_remap.so @pparam=proxy.config.http.normalize_ae_gzip=0'
    )
    ts.Disk.remap_config.AddLine(
        'map http://www.ae-gzip-1.com http://127.0.0.1:{0}'.format(server.Variables.Port) +
        ' @plugin=conf_remap.so @pparam=proxy.config.http.normalize_ae_gzip=1'
    )
...
def perTsTest(shouldWaitForUServer, ts):
    ...
    allAEHdrs(False, False, ts, 'www.ae-gzip-0.com')
    allAEHdrs(False, False, ts, 'www.ae-gzip-1.com')
