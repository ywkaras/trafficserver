...
NOTE: no include guard needed, only included once in ports_ready.h.
*/
...
using PortsReadyHook::InProgress;
...
private:
  ...
  TSIOBufferBlock _io_block;
  char const *_block_data;
};
...
public:
  ...
  {
    if (!_use_port_descriptor_cont.get()) {
  ...
  {
    return _use_port_descriptor_cont.get() != nullptr;
  }
...
private:
  ...
  _contFunc(TSEvent event, void *data)
  {
    ...
    case TS_EVENT_NET_ACCEPT: {
      ...
      //
      static_cast<void>(new VConnLoopback(static_cast<TSVConn>(data), _ip, _use_port_descriptor_cont.get()));
    } break;
  ...
  atscppapi::TSContUniqPtr _use_port_descriptor_cont;
  InProgress _ip;
...
public:
  ...
  {
    TSCont accept_cont{nullptr};
    if (use_port_descriptor) {
      accept_cont = TSContCreate(_acceptContFunc, _mtx);
      TSContDataSet(accept_cont, this);
    }
    _server.reset(new TCPoIPv4LoopbackServer(tcp_port_num, ip, accept_cont));
