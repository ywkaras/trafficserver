...
# test_msg_comp.so will output test logging to this file.
...
# Probe TS cleartext port to check.
tr.Processes.Default.StartBefore(Test.Processes.ts, ready=When.PortOpen(ts.Variables.port))
#
tr.Processes.Default.Command = (
    'curl --header "Connection: close" --verbose --ipv4 --header "Host: one" --header "X-Crap: one, two, three"' +
    ' --header "X-Crap: four" http://localhost:{0}/argh'.format(ts.Variables.port)
)
...
tr = Test.AddTestRun()
tr.Processes.Default.Command = (
    "cd " + Test.RunDirectory + " ; sed 's/{}/SERVER_PORT/' log.txt > log2.txt".format(server.Variables.Port)
)
tr.Processes.Default.ReturnCode = 0
