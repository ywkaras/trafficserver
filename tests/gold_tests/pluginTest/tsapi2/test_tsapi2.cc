...
public:
  ...
  init()
  {
    _map_node = yaml_data["proxy_port_to_txn"];
  }
  ...
  operator std::string const &() const { return _test_id; }
...
private:
  ...
  static inline YAML::Node _map_node;
  ...
  _set_test_id(TSHttpSsn ssnp)
  {
    ...
    auto node = _map_node[ntohs(sock_addr_in->sin_port)];

    TSReleaseAssert(!node.IsNull());

    _test_id = node.as<std::string>();
  }
};
...
TSPluginInit(int argc, const char *argv[])
{
  ...
  yaml_data = YAML::LoadFile(argv[1]);

  run_dir_path     = yaml_data["run_dir_path"].as<std::string>();
  server_port      = yaml_data["server_port"].as<std::uint16_t>();
  mute_server_port = yaml_data["mute_server_port"].as<std::uint16_t>();
  txns             = yaml_data["txns"];
