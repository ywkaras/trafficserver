...
# Define default ATS
ts = Test.MakeATSProcess("ts", select_ports=True, enable_tls=True, extra_ports="upstream_port")
...
ts.Disk.remap_config.AddLine(
    'map /delay-chunked-response http://127.0.0.1:{0}'.format(ts.Variables.upstream_port)
)
ts.Disk.remap_config.AddLine(
    'map / http://127.0.0.1:{0}'.format(ts.Variables.upstream_port)
)
...
tr.Processes.Default.Command = 'sh ./case1.sh {0} {1}'.format(ts.Variables.ssl_port, ts.Variables.upstream_port)
...
tr.Processes.Default.Command = 'sh ./case2.sh {0} {1}'.format(ts.Variables.ssl_port, ts.Variables.upstream_port)
tr.Processes.Default.ReturnCode = 0
...
tr.Processes.Default.Command = 'sh ./case3.sh {0} {1}'.format(ts.Variables.ssl_port, ts.Variables.upstream_port)
tr.Processes.Default.ReturnCode = 0
tr.Processes.Default.Streams.All = Testers.ContainsExpression("HTTP/2 200", "Request should succeed")
