...
should be called after :cpp:enumerator:`TS_HTTP_READ_REQUEST_HDR_HOOK`.

:func:`TSHttpHdrParseResp` operates in the same manner as
:func:`TSHttpHdrParseReq` except it parses an HTTP response header.
It should be called after :cpp:enumerator:`TS_HTTP_READ_RESPONSE_HDR_HOOK`.
...
a :type:`TSParseResult` value. :cpp:enumerator:`TS_PARSE_ERROR` is returned
on error, :cpp:enumerator:`TS_PARSE_CONT` is returned if parsing of the header
stopped because the end of the buffer was reached, and
:cpp:enumerator:`TS_PARSE_DONE` when a \\r\\n\\r\\n pattern is encountered,
