...
:cpp:enumerator:`TS_EVENT_NET_ACCEPT` event. The :arg:`edata` passed with
:cpp:enumerator:`TS_EVENT_NET_ACCEPT` is an :type:`TSVConn` just as it would be for a
...
:cpp:enumerator:`TS_EVENT_NET_ACCEPT_FAILED` event when the transaction completes.
