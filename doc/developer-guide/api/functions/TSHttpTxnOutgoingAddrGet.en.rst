...
done before the outbound connection is made, that is, earlier than the :cpp:enumerator:`TS_HTTP_SEND_REQUEST_HDR_HOOK`.
A good choice is the :cpp:enumerator:`TS_HTTP_POST_REMAP_HOOK`, since it is a hook that is always called, and it
...
:cpp:enumerator:`TS_SUCCESS` on success and :cpp:enumerator:`TS_ERROR` on failure.
...
:cpp:enumerator:`TS_HTTP_SEND_REQUEST_HDR_HOOK` or later, after the outbound connection has been established. It returns a
