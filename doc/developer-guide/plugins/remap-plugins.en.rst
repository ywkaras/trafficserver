...
call :func:`TSRemapInit`. The plugin must return :cpp:enumerator:`TS_SUCCESS` or the configuration loading
...
If :func:`TSRemapInit` returns :cpp:enumerator:`TS_ERROR` then the remap configuration loading
...
If there is an error then the callback should return :cpp:enumerator:`TS_ERROR` and fill in the
:arg:`errbuff` with a C-string describing the error. Otherwise the function must return
:cpp:enumerator:`TS_SUCCESS`.

If :func:`TSRemapNewInstance` returns :cpp:enumerator:`TS_ERROR` then the remap configuration loading
is aborted immediately.
