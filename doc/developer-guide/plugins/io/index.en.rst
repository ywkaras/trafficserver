...
cpp:func:`TSNetConnect`). In the case of transform plugins, the plugin
creates a transformation vconnection viav :cpp:func:`TSTransformCreate`
and then accesses the output vconnection using
:cpp:func:`TSTransformOutputVConnGet`.
...
To issue a read or write operation, a user calls :cpp:func:`TSVConnRead` or
:cpp:func:`TSVConnWrite`. These two operations both return ``VIO (TSVIO)``. The
...
vconnection by calling :cpp:func:`TSVConnWrite`.
...
issuing repeated calls to :manpage:`read(2)` until one of the calls finally
...
calls to :manpage:`read(2)`, but the interface does not expose that detail).
...
One obvious issue is that the buffer passed to :cpp:func:`TSVConnRead` and
:cpp:func:`TSVConnWrite` won't be large enough - there is no reasonable way to
...
the user (via the continuation passed to :cpp:func:`TSVConnRead` and
:cpp:func:`TSVConnWrite`) that they have emptied the buffers passed to them and
...
wake up the vconnection by calling :cpp:func:`TSVIOReenable` on the VIO
describing the operation. :cpp:func:`TSVIOReenable` specifies that the buffer
...
is a prototype for :cpp:func:`TSVConnWrite`:
...
is obtained from ``input_vio`` by :cpp:func:`TSVIONBytesGet`.
...
:cpp:func:`TSContCall`. It passes the ``TSVIO`` describing the operation as the
...
``TS_EVENT_VCONN_READ_READY``
    ...
    :cpp:func:`TSVConnRead` operation and it would like to do more IO, but the
...
``TS_EVENT_VCONN_WRITE_READY``
    ...
    :cpp:func:`TSVConnWrite` operation and it would like to do more IO, but the
...
``TS_EVENT_VCONN_READ_COMPLETE``
    ...
    :cpp:func:`TSVConnRead` operation. The vconnection can now be used to
    initiate a new IO operation.

``TS_EVENT_VCONN_WRITE_COMPLETE``
    The vconnection has written all the bytes specified by an
    :cpp:func:`TSVConnWrite` operation and can now be used to initiate a new IO
    operation.

``TS_EVENT_VCONN_EOS``
    An attempt was made to read past the end of the stream of bytes
    during the handling of an :cpp:func:`TSVConnRead` operation. This event
    ...
    should be read from the vconnection in a call to :cpp:func:`TSVConnRead`. A
...
:cpp:func:`TSVConnWrite`.

After using a vconnection, the user must call :cpp:func:`TSVConnClose` or
:cpp:func:`TSVConnAbort`. While both calls indicate that the vconnection can
destroy itself, :cpp:func:`TSVConnAbort` should be used when the connection is
being closed abnormally. After a call to :cpp:func:`TSVConnClose` or
:cpp:func:`TSVConnAbort`, the user will not be called back by the vconnection
again.
...
via the :cpp:func:`TSVConnShutdown` function, which shuts down either the read
...
:cpp:func:`TSVConnShutdown`. To learn how vconnections are used in
...
-  :cpp:func:`TSVConnAbort`
-  :cpp:func:`TSVConnClose`
-  :cpp:func:`TSVConnClosedGet`
-  :cpp:func:`TSVConnRead`
-  :cpp:func:`TSVConnReadVIOGet`
-  :cpp:func:`TSVConnShutdown`
-  :cpp:func:`TSVConnWrite`
-  :cpp:func:`TSVConnWriteVIOGet`
