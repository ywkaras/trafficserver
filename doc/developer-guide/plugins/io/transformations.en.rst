...
examining the VIO returned by a call to :cpp:func:`TSVConnRead` or
:cpp:func:`TSVConnWrite`. The implementer, in turn, gets a handle on the VIO
operation by examining the VIO returned by :cpp:func:`TSVConnReadVIOGet` or
:cpp:func:`TSVConnWriteVIOGet` (recall that every vconnection created through
...
why :cpp:func:`TSVConnClose` does not simply just destroy the vconnection.
...
:cpp:func:`TSVConnRead`, :cpp:func:`TSVConnWrite`, :cpp:func:`TSVConnClose`,
:cpp:func:`TSVConnShutdown`, or :cpp:func:`TSVIOReenable` call is performed.
:cpp:func:`TSVIOReenable` indirectly references the vconnection through a
...
the state of its VIOs. For example, when :cpp:func:`TSVConnClose` is called, the
...
to see if it has been closed. Similarly, when :cpp:func:`TSVIOReenable` is
...
-  After reading or copying data, make sure that you consume the data
   ...
   following example taken from ``null_transform.cc``:
...
-  Before sending ``TS_EVENT_VCONN_WRITE_COMPLETE``, your transformation
   ...
   ``TS_EVENT_VCONN_WRITE_COMPLETE`` events if :cpp:func:`TSVIONTodoGet` is
   greater than zero.
-  The transformation passes data out of itself by using the output
   vconnection retrieved by :cpp:func:`TSTransformOutputVConnGet`. Immediately
...
-  All of the transformations in a transformation chain share the
   ...
   :cpp:func:`TSTransformCreate`) removes many of the locking complications of
