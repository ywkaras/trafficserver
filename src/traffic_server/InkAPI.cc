...
INKContInternal::destroy()
{
  ...
  if (m_deletable) {
    ...
    // If this_thread() returns null, the Ethrread object for the current object has been destroyed.  Presumably this
    // will only happen during destruction of statically-initialized objects at TS shutdown, so no further action is
    // needed.
    //
    if (p) {
      p->schedule_imm(this);
