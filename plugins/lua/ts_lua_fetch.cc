...
ts_lua_fetch_one_item(lua_State *L, const char *url, size_t url_len, ts_lua_fetch_info *fi)
{
  ...
  /* Host */
  if (ht == 0) {
    ptr = static_cast<char const *>(memchr(url, ':', url_len));

    if (ptr) {
      host = ptr + 3;
      left = url_len - (host - url);

      ptr = static_cast<char const *>(memchr(host, '/', left));
...
ts_lua_fetch_handler(TSCont contp, TSEvent ev, void *edata ATS_UNUSED)
{
  ...
  event = (int)ev;
  fi    = static_cast<decltype(fi)>(TSContDataGet(contp));
  fmi   = fi->fmi;
  ...
  if (fmi && (fi->over || fi->failed)) {
    TSContCall(fmi->contp, TSEvent(TS_LUA_EVENT_FETCH_OVER), fi); // error exist
    ts_lua_destroy_fetch_multi_info(fmi);
  }
