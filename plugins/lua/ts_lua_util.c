...
ts_lua_destroy_http_ctx(ts_lua_http_ctx *http_ctx)
{
  ...
  if (http_ctx->rri == NULL) {
    if (http_ctx->client_request_bufp) {
      TSMimeHdrFldRelease(http_ctx->client_request_bufp, nullptr, http_ctx->client_request_hdrp);
    }
  }

  if (http_ctx->server_request_url) {
    TSMimeHdrFldRelease(http_ctx->server_request_bufp, http_ctx->server_request_hdrp, http_ctx->server_request_url);
  }

  if (http_ctx->server_request_bufp) {
    TSMimeHdrFldRelease(http_ctx->server_request_bufp, nullptr, http_ctx->server_request_hdrp);
  }

  if (http_ctx->server_response_bufp) {
    TSMimeHdrFldRelease(http_ctx->server_response_bufp, nullptr, http_ctx->server_response_hdrp);
  }

  if (http_ctx->client_response_bufp) {
    TSMimeHdrFldRelease(http_ctx->client_response_bufp, nullptr, http_ctx->client_response_hdrp);
  }

  if (http_ctx->cached_response_bufp) {
    TSMimeHdrDestroy(http_ctx->cached_response_bufp, http_ctx->cached_response_hdrp);
    TSMimeHdrFldRelease(http_ctx->cached_response_bufp, nullptr, http_ctx->cached_response_hdrp);
