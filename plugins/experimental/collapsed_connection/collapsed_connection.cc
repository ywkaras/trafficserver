 ...
 * @param TSHttpHdrLoc    hdr_loc TS header loc
 *
 * @return uint32_t hash_key
 */
static uint32_t
getCacheUrlHashKey(TSHttpTxn txnp, TSMBuffer bufp, TSHttpHdrLoc /* hdr_loc ATS_UNUSED */)
{
 ...
 */
static bool
isResponseCacheable(TSMBuffer bufp, TSMimeHdrLoc hdr_loc)
{
  bool cacheable    = false;
  bool found_public = false;
  bool found_maxage = false;
  bool found_expire = false;
  TSMimeHdrLoc field_loc  = nullptr;
